  ...
  <div class="table-responsive">
    <table id="tblType" class="table table-bordered table-hover align-middle" style="width:100%">
      <thead><tr>
        ...
        <th class="cfp-col-hide text-center" style="width:100px">จัดการ</th>
      </tr></thead>
      <tbody>
      ...
      <tr data-status="<?php echo $r['IsActive']?'1':'0'; ?>">
        ...
        <td class="cfp-col-hide text-muted" style="font-size:0.7rem;color:#6c757d;"><?php echo htmlspecialchars($r['Description']??''); ?></td>
        <td class="cfp-col-hide text-center"><?php echo $r['SortOrder']; ?></td>
        ...
        <td class="cfp-col-hide text-center">
          <div class="cfp-action-group">
            <button class="btn btn-outline-primary btn-action me-1 cfp-act-primary" onclick="openModal(<?php echo (int)$r['TypeID']; ?>)" title="แก้ไข"><i class="bi bi-pencil-square"></i><span class="cfp-act-label">แก้ไข</span></button>
            <div class="cfp-act-secondary">
              <button class="btn btn-action <?php echo $r['IsActive']?'btn-outline-danger':'btn-outline-success'; ?> me-1 cfp-act-toggle"
                ...
                <i class="bi bi-<?php echo $r['IsActive']?'toggle2-off':'toggle2-on'; ?>"></i><span class="cfp-act-label"><?php echo $r['IsActive']?'ปิดใช้งาน':'เปิดใช้งาน'; ?></span></button>
              <button class="btn btn-outline-warning btn-action cfp-act-del" onclick="confirmDelete(<?php echo (int)$r['TypeID']; ?>,'<?php echo htmlspecialchars(addslashes($r['TypeName'])); ?>')" title="ลบ">
                <i class="bi bi-trash"></i><span class="cfp-act-label">ลบ</span></button>
            </div>
          </div>
        </td>
      </tr>
...
$(document).ready(function(){
  tblApi=$('#tblType').DataTable({language:{url:'https://cdn.datatables.net/plug-ins/1.13.6/i18n/th.json'},order:[[5,'asc']],pageLength:25,dom:'<"row align-items-center mb-2"<"col-auto"l><"col">>rtip',columnDefs:[{targets:0,orderable:false,searchable:false}],drawCallback:function(){cfpInitMobileExpand('tblType');}});
