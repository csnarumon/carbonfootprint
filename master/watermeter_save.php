<?php
/* master/watermeter_save.php
   Table: CFP_WaterMeter — มิเตอร์น้ำ
...
if ($action === 'toggle') {
    if ($id <= 0) {
        ...
        header('Location: watermeter.php'); exit;
    }
    ...
    sqlsrv_query($conn,
        "UPDATE CFP_WaterMeter SET IsActive=?,UpdatedBy=?,UpdatedDate=GETDATE() WHERE MeterID=?",
        array($newVal, $userID, $id));
    logAction($conn, 'DATA_UPDATE', 'CFP_WaterMeter', $id, null, null, null, $label . 'มิเตอร์น้ำ ID=' . $id);

    header('Content-Type: text/html; charset=utf-8');
    $_SESSION['toast_msg']  = $label . 'มิเตอร์น้ำเรียบร้อยแล้ว';
...
$mTypeID = (int)($_POST['WaterMeterTypeID'] ?? 0) ?: null;
$wSrcID  = (int)($_POST['WaterSourceID'] ?? 0) ?: null;
...
if (empty($name)) {
    jsonOut(false, 'กรุณากรอกชื่อมิเตอร์น้ำ');
}
...
if (empty($code) && $siteID) {
    ...
    $rMax = sqlsrv_query($conn,
        "SELECT MAX(MeterCode) AS MaxCode FROM CFP_WaterMeter WHERE MeterCode LIKE ?",
        array('WM-' . $sc . '-%'));
    ...
    $code = 'WM-' . $sc . '-' . str_pad($nextNum, 3, '0', STR_PAD_LEFT);
}

/* ---------- CREATE ---------- */
if ($action === 'create') {
    $chk = sqlsrv_query($conn, "SELECT 1 FROM CFP_WaterMeter WHERE MeterCode=?", array($code));
    if (sqlsrv_fetch($chk)) { jsonOut(false, 'รหัส "' . $code . '" มีอยู่ในระบบแล้ว'); }

    $res = sqlsrv_query($conn,
        "INSERT INTO CFP_WaterMeter (MeterCode,MeterName,SiteID,WaterMeterTypeID,WaterSourceID,MeterNo,InstallDate,Location,Remark,IsActive,CreatedBy,CreatedDate)
         VALUES (?,?,?,?,?,?,?,?,?,1,?,GETDATE())",
        array($code,$name,$siteID,$mTypeID,$wSrcID,$meterNo?:null,$install,$loc?:null,$remark?:null,$userID));
    if (!$res) {
        $e = sqlsrv_errors();
    ...
    list($fc, $fe) = handleFiles($conn, $userID, $newID, 'WaterMeter', 'watermeter');
    logAction($conn, 'DATA_CREATE', 'CFP_WaterMeter', $newID, null, null, null, 'เพิ่ม: ' . $code . ' ' . $name);
    
    $msg = 'เพิ่มมิเตอร์น้ำ "' . $name . '" เรียบร้อยแล้ว (ไฟล์ ' . $fc . ' รายการ)';
...
if ($action === 'update') {
    ...
    $chk = sqlsrv_query($conn,
        "SELECT 1 FROM CFP_WaterMeter WHERE MeterCode=? AND MeterID<>?", array($code, $id));
    if (sqlsrv_fetch($chk)) { jsonOut(false, 'รหัส "' . $code . '" มีอยู่ในระบบแล้ว'); }

    $res = sqlsrv_query($conn,
        "UPDATE CFP_WaterMeter SET MeterCode=?,MeterName=?,SiteID=?,WaterMeterTypeID=?,WaterSourceID=?,MeterNo=?,InstallDate=?,Location=?,Remark=?,UpdatedBy=?,UpdatedDate=GETDATE()
         WHERE MeterID=?",
        array($code,$name,$siteID,$mTypeID,$wSrcID,$meterNo?:null,$install,$loc?:null,$remark?:null,$userID,$id));
    if (!$res) {
        $e = sqlsrv_errors();
    ...
    if ($pendingDeletesRaw !== '') {
        ...
        foreach ($deleteIDs as $delID) {
            ...
            $rowDel = sqlsrv_fetch_array(
                sqlsrv_query($conn, "SELECT FileName FROM CFP_AssetImage WHERE ImageID=? AND AssetType='WaterMeter' AND AssetID=?", array($delID, $id)),
                SQLSRV_FETCH_ASSOC
            );
            if ($rowDel) {
                $delFile = dirname(__DIR__) . '/uploads/assets/watermeter/' . $rowDel['FileName'];
    ...
    list($fc, $fe) = handleFiles($conn, $userID, $id, 'WaterMeter', 'watermeter');
    logAction($conn, 'DATA_UPDATE', 'CFP_WaterMeter', $id, null, null, null, 'แก้ไข: ' . $code . ' ' . $name);
    
    $msg = 'แก้ไขมิเตอร์น้ำ "' . $name . '" เรียบร้อยแล้ว (ไฟล์ ' . $fc . ' รายการ)';
    if ($fe) { $msg .= ' มีปัญหา: '.implode('; ',$fe); }
