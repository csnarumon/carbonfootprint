        ...
        <ul class="nav nav-tabs mb-3" id="userTabs">
          <li class="nav-item">
            <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tabUsers">
              ...
              <span class="badge bg-secondary ms-1" style="font-size:0.7rem;"><?php echo $total; ?></span>
            </button>
          </li>
          <li class="nav-item">
            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabScope">
              <i class="bi bi-shield-shaded me-1"></i>สิทธิ์ Data Entry
              <?php if ($noScopeCount > 0) { ?>
              <span class="badge ms-1" style="background:#E05050;font-size:0.7rem;"><?php echo $noScopeCount; ?></span>
              <?php } ?>
            </button>
          </li>
          <li class="nav-item">
            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabReviewer">
              ...
              if ($noReviewerSite > 0) { ?>
              <span class="badge ms-1" style="background:#E05050;font-size:0.7rem;"><?php echo $noReviewerSite; ?></span>
              <?php } ?>
            </button>
          </li>
          <li class="nav-item">
    ...
    <div class="modal-content font-prompt">
      ...
      <div class="modal-body">
        ...
        <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-bottom:8px;">
          ...
          <span style="color:#B91C1C;">(ไม่เลือก = ไม่เห็นข้อมูลใด)</span>
        </div>
...
function saveScopeAccess() {
    ...
    [1,2,3].forEach(function(sNo) {
        if (document.getElementById('chkScope' + sNo).checked) {
            var catIDs = '';
            if (sNo === 3) {
                /* เก็บเฉพาะที่เลือก ถ้าไม่เลือกเลย = ทุก cat (ว่าง) */
                var checked = [];
...
function saveReviewerSite() {
    var selected = [];
    document.querySelectorAll('.rev-site-chk:checked').forEach(function(c) { selected.push(c.value); });
    document.getElementById('fRevSiteUserID').value = currentReviewerUserID;
    document.getElementById('fRevSiteIDs').value    = selected.join(',');
    document.getElementById('formReviewerSite').submit();
}
/* ===== Copy User ===== */
