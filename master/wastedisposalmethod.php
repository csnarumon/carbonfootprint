<?php
/* ==============================================
   master/wastedisposalmethod.php
   จัดการวิธีกำจัดขยะ — Admin (4), SustainAdmin (5)
   ============================================== */
...
/* ===== ดึงรายการวิธีกำจัดขยะ ===== */
$res = sqlsrv_query($conn, "
    SELECT MethodID, MethodCode, MethodName, Description, SortOrder, IsActive
    FROM CFP_WasteDisposalMethod
    ORDER BY SortOrder, MethodName");
$rows = array();
...
/* นับจำนวนวิธีที่ถูกใช้งานจริงใน CFP_Waste (มี DisposalMethodID อ้างอิงอยู่อย่างน้อย 1 รายการ) */
$usedCount = 0;
$resUsed = @sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT DisposalMethodID) AS Cnt
    FROM CFP_Waste
    WHERE DisposalMethodID IS NOT NULL");
if ($resUsed !== false) {
    $rowUsed  = sqlsrv_fetch_array($resUsed, SQLSRV_FETCH_ASSOC);
    $usedCount = $rowUsed ? (int)$rowUsed['Cnt'] : 0;
}

/* จำนวนที่นำไปใช้จริงแยกตาม MethodID — ใช้ตัดสินใจว่าวิธีไหนลบได้ปลอดภัย */
$usageByType = array();
$resUsageDetail = @sqlsrv_query($conn, "
    SELECT DisposalMethodID, COUNT(*) AS Cnt FROM CFP_Waste
    WHERE DisposalMethodID IS NOT NULL GROUP BY DisposalMethodID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['DisposalMethodID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>วิธีกำจัดขยะ — ระบบบริหารจัดการคาร์บอนองค์กร</title>
     ...
     <!-- Page Header -->
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
          <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการวิธีกำจัดขยะ
          </h5>
          <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
            การตั้งค่าข้อมูลพื้นฐาน › วิธีกำจัดขยะ
          </div>
        </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-arrow-repeat" style="color:#1565C0;"></i>
            </div>
            <div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-arrow-repeat" style="color:#E65100;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#E65100;line-height:1.1;">
                <?php echo $usedCount; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">ถูกใช้งานในข้อมูลขยะ</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-arrow-repeat me-2"></i>รายการวิธีกำจัดขยะ
        </div>
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" style="background:var(--cfp-primary);color:#fff;">
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มวิธีกำจัดขยะ
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="wastedisposalmethod_save.php">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <input type="hidden" name="action" id="fAction" value="create">
        <input type="hidden" name="MethodID" id="fID" value="0">
  ...
  <input type="text" class="form-control font-prompt" id="fCodeDisplay"
         ...
         style="background:#F0F0F0;color:var(--cfp-text-muted);">
            ...
            <div class="col-md-7">
              ...
              <input type="text" class="form-control font-prompt" name="MethodName" id="fName"
                     placeholder="เช่น ฝังกลบ, เผา, รีไซเคิล" maxlength="200" required>
            </div>
            <div class="col-12">
  ...
  <div class="modal-dialog">
    <div class="modal-content font-prompt"> 
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import วิธีกำจัดขยะ จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="wastedisposalmethod_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="wastedisposalmethod_template.php" class="cfp-import-template-link">
...
คอลัมน์: ชื่อ*, คำอธิบาย, ลำดับ (รหัสระบบสร้างให้อัตโนมัติ)
</p>
          ...
          <div class="mt-2" style="font-size:0.78rem;color:var(--cfp-text-muted);">
            ...
            <ul class="mb-0 ps-3">
             ...
             <li>รหัสประเภทระบบสร้างให้อัตโนมัติ ไม่ต้องกรอกในไฟล์<!-- Badge สีส้ม  -->
...
<!-- Hidden Form สำหรับลบแบบ POST — ป้องกัน csrf_token ติด URL/history -->
<form id="formDelete" method="POST" action="wastedisposalmethod_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
  <input type="hidden" name="action" value="delete">
  <input type="hidden" name="MethodID" id="fdID" value="0">
</form>

<!-- Hidden Form สำหรับเปิด/ปิดใช้งานแบบ POST -->
<form id="formToggle" method="POST" action="wastedisposalmethod_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
  <input type="hidden" name="action" value="toggle">
  <input type="hidden" name="MethodID" id="ftID" value="0">
</form>
...
$(document).ready(function () {
    ...
    /* status filter: redraw ทุกครั้งที่เปลี่ยน */
...
function openModal(id) {
    ...
    if (id === 0) {
        // เพิ่มใหม่
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มวิธีกำจัดขยะ';
        ...
        // แก้ไข
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขวิธีกำจัดขยะ';
...
function confirmDelete(id, name) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            /* ส่งแบบ POST ผ่าน form ที่ซ่อนไว้ — csrf_token ไม่ติดไปกับ URL/history */
...
/* Validate ก่อนส่งฟอร์ม Import — ถ้ายังไม่เลือกไฟล์ เตือนด้วย SweetAlert */
