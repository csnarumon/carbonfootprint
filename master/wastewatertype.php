<?php
/* ==============================================
   master/wastewatertype.php
   จัดการประเภทการปล่อยน้ำเสีย — Admin (4), SustainAdmin (5)
   ============================================== */
...
/* ===== ดึงรายการประเภทการปล่อยน้ำเสีย ===== */
...
/* นับจำนวนประเภทที่ถูกใช้งานจริงใน CFP_Wastewater (มี WastewaterTypeID อ้างอิงอยู่อย่างน้อย 1 รายการ) */
$usedCount = 0;
$resUsed = @sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT WastewaterTypeID) AS Cnt
    FROM CFP_Wastewater
    WHERE WastewaterTypeID IS NOT NULL");
if ($resUsed !== false) {
...
$resUsageDetail = @sqlsrv_query($conn, "
    SELECT WastewaterTypeID, COUNT(*) AS Cnt FROM CFP_Wastewater
    WHERE WastewaterTypeID IS NOT NULL GROUP BY WastewaterTypeID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['WastewaterTypeID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>ประเภทการปล่อยน้ำเสีย — ระบบบริหารจัดการคาร์บอนองค์กร</title>
...
<div class="d-flex">
  ...
  <div class="cfp-main">
    ...
    <?php
    $pageTitle = 'ประเภทการปล่อยน้ำเสีย';
    ...
    <!-- Page Header -->
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
          <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการประเภทการปล่อยน้ำเสีย
          </h5>
          <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
            การตั้งค่าข้อมูลพื้นฐาน › ประเภทการปล่อยน้ำเสีย
          </div>
        </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-droplet-half" style="color:#1565C0;"></i>
            </div>
            <div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-droplet-half" style="color:#E65100;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#E65100;line-height:1.1;">
                <?php echo $usedCount; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">ถูกใช้งานในน้ำเสีย</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-droplet-half me-2"></i>รายการประเภทการปล่อยน้ำเสีย
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มประเภทการปล่อยน้ำเสีย
            </button>
          </div>
        </div>
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" style="background:var(--cfp-primary);color:#fff;">
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทการปล่อยน้ำเสีย
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="wastewatertype_save.php">
         ...
         style="background:#F0F0F0;color:var(--cfp-text-muted);">
            ...
            <div class="col-md-7">
              ...
              <input type="text" class="form-control font-prompt" name="TypeName" id="fName"
                     placeholder="เช่น น้ำเสียอุตสาหกรรม" maxlength="200" required>
            </div>
            <div class="col-12">
  ...
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import ประเภทการปล่อยน้ำเสีย จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="wastewatertype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="wastewatertype_template.php" class="cfp-import-template-link">
          ...
          <div class="cfp-import-dropzone" id="dropZone" onclick="document.getElementById('importFile').click()">
            ...
            <p style="font-size:0.72rem;color:var(--cfp-text-muted);">คอลัมน์: ชื่อ*, คำอธิบาย, ลำดับ (รหัสระบบสร้างให้อัตโนมัติ)</p>
...
<form id="formDelete" method="POST" action="wastewatertype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
<form id="formToggle" method="POST" action="wastewatertype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
function openModal(id) {
    ...
    if (id === 0) {
        // เพิ่มใหม่
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทการปล่อยน้ำเสีย';
        ...
        // แก้ไข
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขประเภทการปล่อยน้ำเสีย';
...
function confirmToggle(id, currentActive, name) {
    ...
    Swal.fire({
        title: action + 'ประเภทการปล่อยน้ำเสีย?',
