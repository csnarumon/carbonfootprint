<?php
/* ==============================================
   master/watermetertype_save.php
   รับ POST จาก watermetertype.php
...
if ($action === 'toggle') {
    ...
    if (!$id) { redirectWithToast('ไม่พบข้อมูลประเภทมิเตอร์น้ำ', 'error'); }

    $res = sqlsrv_query($conn, "SELECT IsActive, TypeName FROM CFP_WaterMeterType WHERE TypeID=?", array($id));
    $row = sqlsrv_fetch_array($res, SQLSRV_FETCH_ASSOC);
    if (!$row) { redirectWithToast('ไม่พบข้อมูลประเภทมิเตอร์น้ำ', 'error'); }

    $newStatus = $row['IsActive'] ? 0 : 1;

    sqlsrv_query($conn, "UPDATE CFP_WaterMeterType SET IsActive=?, UpdatedBy=?, UpdatedDate=GETDATE() WHERE TypeID=?",
        array($newStatus, (int)$_SESSION['user_id'], $id));

    logAction($conn, 'DATA_UPDATE', 'CFP_WaterMeterType', $id, null, null, null,
...
if ($action === 'delete') {
    ...
    /* ตรวจว่ามีการใช้งานอยู่ใน CFP_WaterMeter หรือไม่ (เช็คผ่าน FK WaterMeterTypeID) */
    $res = sqlsrv_query($conn, "SELECT COUNT(*) AS Cnt FROM CFP_WaterMeter WHERE WaterMeterTypeID = ?", array($id));
    $chk = sqlsrv_fetch_array($res, SQLSRV_FETCH_ASSOC);

    if ($chk && $chk['Cnt'] > 0) {
        /* มีการใช้งานอยู่ → Soft delete (ปิดใช้งาน) แทนการลบจริง */
        sqlsrv_query($conn, "UPDATE CFP_WaterMeterType SET IsActive=0, UpdatedBy=?, UpdatedDate=GETDATE() WHERE TypeID=?",
            array((int)$_SESSION['user_id'], $id));
        logAction($conn, 'DATA_UPDATE', 'CFP_WaterMeterType', $id, null, null, null, 'ปิดใช้งาน (มีการใช้งานอยู่)');
        redirectWithToast('มีมิเตอร์น้ำใช้ประเภทนี้อยู่ ระบบปิดใช้งานให้แทนการลบ');
    }

    $rDel = sqlsrv_query($conn, "DELETE FROM CFP_WaterMeterType WHERE TypeID=?", array($id));
    if ($rDel === false) {
        $err = sqlsrv_errors();
        redirectWithToast('ไม่สามารถลบข้อมูลได้: ' . ($err[0]['message'] ?? 'เกิดข้อผิดพลาดไม่ทราบสาเหตุ'), 'error');
    }
    logAction($conn, 'DATA_DELETE', 'CFP_WaterMeterType', $id);
...
if ($action === 'create') {
    ...
    logAction($conn, 'DATA_CREATE', 'CFP_WaterMeterType', null, null, null, null,
        'เพิ่มประเภทมิเตอร์น้ำ: ' . $code . ' - ' . $name);
    redirectWithToast('เพิ่มประเภทมิเตอร์น้ำเรียบร้อยแล้ว (รหัส ' . $code . ')');
