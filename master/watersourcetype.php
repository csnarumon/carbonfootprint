<?php
/* ==============================================
   master/watersourcetype.php
   จัดการแหล่งน้ำ — Admin (4), SustainAdmin (5)
...
$res = sqlsrv_query($conn, "
    SELECT SourceID, SourceCode, SourceName, Description, SortOrder, IsActive
...
/* นับจำนวนแหล่งน้ำที่ถูกใช้งานจริงใน CFP_WaterMeter */
$resUsed = sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT WaterSourceID) AS Cnt
    FROM CFP_WaterMeter
    WHERE WaterSourceID IS NOT NULL");
$rowUsed   = sqlsrv_fetch_array($resUsed, SQLSRV_FETCH_ASSOC);
$usedCount = $rowUsed ? (int)$rowUsed['Cnt'] : 0;

/* จำนวนที่นำไปใช้จริงแยกตาม SourceID — ใช้ตัดสินใจว่าแหล่งไหนลบได้ปลอดภัย */
$usageByType = array();
$resUsageDetail = sqlsrv_query($conn, "
    SELECT WaterSourceID, COUNT(*) AS Cnt FROM CFP_WaterMeter
    WHERE WaterSourceID IS NOT NULL GROUP BY WaterSourceID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['WaterSourceID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>แหล่งน้ำ — ระบบบริหารจัดการคาร์บอนองค์กร</title>
    ...
    <div class="cfp-content">
      ...
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
          <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการแหล่งน้ำ
          </h5>
          <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
            การตั้งค่าข้อมูลพื้นฐาน › แหล่งน้ำ
          </div>
        </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-droplet" style="color:#1565C0;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:var(--cfp-primary);line-height:1.1;">
                <?php echo $total; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">แหล่งน้ำทั้งหมด</div>
            </div>
          </div>
        </div>
        <div class="col-6 col-md-3">
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-water" style="color:#E65100;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#E65100;line-height:1.1;">
                <?php echo $usedCount; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">ถูกใช้งานในมิเตอร์น้ำ</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-droplet me-2"></i>รายการแหล่งน้ำ
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          <div class="d-flex gap-2 flex-grow-1" style="max-width:560px;">
            ...
            <input type="text" id="fltKeyword" class="form-control font-prompt" style="font-size:0.85rem;padding-right:28px;"
                   placeholder="ค้นหารหัส / ชื่อแหล่งน้ำ...">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มแหล่งน้ำ
            </button>
          </div>
        </div>
          ...
          <table id="tblType" class="table table-bordered table-hover align-middle" style="width:100%">
            <thead>
              <tr>
                ...
                <th style="min-width:180px;">ชื่อแหล่งน้ำ</th>
                <th>คำอธิบาย</th>
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มแหล่งน้ำ
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="watersourcetype_save.php">
        ...
        <div class="modal-body">
          <div class="row g-3">
            <div class="col-md-5">
              <label class="form-label">รหัสแหล่งน้ำ</label>
            ...
            <div class="col-md-7">
              <label class="form-label form-required">ชื่อแหล่งน้ำ</label>
              <input type="text" class="form-control font-prompt" name="SourceName" id="fName"
                     placeholder="เช่น น้ำประปา" maxlength="200" required>
            </div>
            <div class="col-12">
...
<div class="modal fade modal-cfp" id="modalImport" tabindex="-1" data-bs-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import แหล่งน้ำ จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="watersourcetype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="watersourcetype_template.php" class="cfp-import-template-link">
...
<form id="formDelete" method="POST" action="watersourcetype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
<form id="formToggle" method="POST" action="watersourcetype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
function openModal(id) {
    ...
    if (id === 0) {
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มแหล่งน้ำ';
        document.getElementById('fAction').value = 'create';
        document.getElementById('fID').value     = '0';
        document.getElementById('fCodeDisplay').value = 'ระบบสร้างให้อัตโนมัติ';
    } else {
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขแหล่งน้ำ';
...
function confirmToggle(id, currentActive, name) {
    ...
    Swal.fire({
        title: action + 'แหล่งน้ำ?',
        html: 'แหล่งน้ำ: <b>' + name + '</b>',
        icon: icon,
        showCancelButton: true,
