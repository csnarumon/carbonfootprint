<?php
/* ==============================================
   master/flourwastetype.php
   จัดการประเภทการกำจัดเศษแป้ง — Admin (4), SustainAdmin (5)
   ============================================== */
...
/* ===== ดึงรายการประเภทการกำจัดเศษแป้ง ===== */
...
/* นับจำนวนประเภทที่ถูกใช้งานจริงใน CFP_FlourWaste (มี FlourWasteTypeID อ้างอิงอยู่อย่างน้อย 1 รายการ) */
$usedCount = 0;
$resUsed = @sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT FlourWasteTypeID) AS Cnt
    FROM CFP_FlourWaste
    WHERE FlourWasteTypeID IS NOT NULL");
if ($resUsed !== false) {
...
$resUsageDetail = @sqlsrv_query($conn, "
    SELECT FlourWasteTypeID, COUNT(*) AS Cnt FROM CFP_FlourWaste
    WHERE FlourWasteTypeID IS NOT NULL GROUP BY FlourWasteTypeID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['FlourWasteTypeID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>ประเภทการกำจัดเศษแป้ง — ระบบบริหารจัดการคาร์บอนองค์กร</title>
...
<div class="d-flex">
  ...
  <div class="cfp-main">
    ...
    <?php
    $pageTitle = 'ประเภทการกำจัดเศษแป้ง';
    $pageIcon  = 'recycle';
    ...
    <!-- Page Header -->
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
          <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการประเภทการกำจัดเศษแป้ง
          </h5>
          <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
            การตั้งค่าข้อมูลพื้นฐาน › ประเภทการกำจัดเศษแป้ง
          </div>
        </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-recycle" style="color:#1565C0;"></i>
            </div>
            <div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-recycle" style="color:#E65100;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#E65100;line-height:1.1;">
                <?php echo $usedCount; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">ถูกใช้งานในเศษแป้ง</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-recycle me-2"></i>รายการประเภทการกำจัดเศษแป้ง
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มประเภทการกำจัดเศษแป้ง
            </button>
          </div>
        </div>
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" style="background:var(--cfp-primary);color:#fff;">
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทการกำจัดเศษแป้ง
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="flourwastetype_save.php">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-7">
              ...
              <input type="text" class="form-control font-prompt" name="TypeName" id="fName"
                     placeholder="เช่น นำไปเป็นวัตถุดิบ" maxlength="200" required>
            </div>
            <div class="col-12">
  ...
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import ประเภทการกำจัดเศษแป้ง จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="flourwastetype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="flourwastetype_template.php" class="cfp-import-template-link">
          ...
          <div id="importFileName" style="margin-top:10px;font-size:0.82rem;color:var(--cfp-primary);"></div>
           ...
           <div class="mt-2" style="font-size:0.78rem;color:var(--cfp-text-muted);">
            ...
            <ul class="mb-0 ps-3">
              ...
              <li>ระบบสร้างรหัสแหล่งน้ำให้อัตโนมัติทุกแถว (<?php echo CODE_PREFIX; ?>-0001, <?php echo CODE_PREFIX; ?>-0002, ...)
              <!-- Badge สีส้ม  -->
...
<form id="formDelete" method="POST" action="flourwastetype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
<form id="formToggle" method="POST" action="flourwastetype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
function openModal(id) {
    ...
    if (id === 0) {
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทการกำจัดเศษแป้ง';
        document.getElementById('fAction').value = 'create';
        document.getElementById('fID').value     = '0';
        document.getElementById('fCodeDisplay').value = 'ระบบสร้างให้อัตโนมัติ';
    } else {
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขประเภทการกำจัดเศษแป้ง';
...
function confirmToggle(id, currentActive, name) {
    ...
    Swal.fire({
        title: action + 'ประเภทการกำจัดเศษแป้ง?',
