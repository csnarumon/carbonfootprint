<?php
/* ==============================================
   master/wastetype.php
   จัดการประเภทขยะของเสีย — Admin (4), SustainAdmin (5)
   ============================================== */
...
/* ===== ดึงรายการประเภทขยะของเสีย ===== */
$res = sqlsrv_query($conn, "
    SELECT TypeID, TypeCode, TypeName, WasteGroup, Description, SortOrder, IsActive
    FROM CFP_WasteType
...
<head>
  ...
  <title>ประเภทขยะของเสีย — ระบบบริหารจัดการคาร์บอนองค์กร</title>
    ...
    <!-- Page Header -->
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
          <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการประเภทขยะของเสีย
          </h5>
          <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
            การตั้งค่าข้อมูลพื้นฐาน › ประเภทขยะของเสีย
          </div> 
        </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-trash" style="color:#1565C0;"></i>
            </div>
            <div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-trash" style="color:#E65100;"></i>
            </div>
            <div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-trash me-2"></i>รายการประเภทขยะของเสีย
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มประเภทขยะของเสีย
            </button>
          </div>
        </div>
          ...
          <table id="tblType" class="table table-bordered table-hover align-middle" style="width:100%">
            <thead>
              <tr>
                ...
                <th class="cfp-col-hide" style="width:140px;">กลุ่มของเสีย</th>
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" style="background:var(--cfp-primary);color:#fff;">
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทขยะของเสีย
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="wastetype_save.php">
         ...
         style="background:#F0F0F0;color:var(--cfp-text-muted);">
            ...
            <div class="col-md-7">
              ...
              <input type="text" class="form-control font-prompt" name="TypeName" id="fName"
                     placeholder="เช่น ขยะอินทรีย์" maxlength="200" required>
            </div>
            <div class="col-md-6">
              <label class="form-label">กลุ่มของเสีย</label>
              <input type="text" class="form-control font-prompt" name="WasteGroup" id="fGroup"
                     placeholder="เช่น Organic, Hazardous" maxlength="100">
            </div>
            <div class="col-md-6">
  ...
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import ประเภทขยะของเสีย จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="wastetype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="wastetype_template.php" class="cfp-import-template-link">
...
<form id="formDelete" method="POST" action="wastetype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
<form id="formToggle" method="POST" action="wastetype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
