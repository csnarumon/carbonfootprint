<?php
/* ==============================================
   master/watermetertype.php
   จัดการประเภทมิเตอร์น้ำ — Admin (4), SustainAdmin (5)
   ============================================== */
...
/* นับจำนวนประเภทที่ถูกใช้งานจริงใน CFP_WaterMeter (มี WaterMeterTypeID อ้างอิงอยู่อย่างน้อย 1 ตัว) */
$resUsed = sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT WaterMeterTypeID) AS Cnt
    FROM CFP_WaterMeter
    WHERE WaterMeterTypeID IS NOT NULL");
...
$resUsageDetail = sqlsrv_query($conn, "
    SELECT WaterMeterTypeID, COUNT(*) AS Cnt FROM CFP_WaterMeter
    WHERE WaterMeterTypeID IS NOT NULL GROUP BY WaterMeterTypeID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['WaterMeterTypeID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>ประเภทมิเตอร์น้ำ — ระบบบริหารจัดการคาร์บอนองค์กร</title>
     ...
     <!-- Page Header -->
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
          <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการประเภทมิเตอร์น้ำ
          </h5>
          <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
            การตั้งค่าข้อมูลพื้นฐาน › ประเภทมิเตอร์น้ำ
          </div>
        </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-speedometer2" style="color:#1565C0;"></i>
            </div>
            <div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-speedometer2 me-2"></i>รายการประเภทมิเตอร์น้ำ
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มประเภทมิเตอร์น้ำ
            </button>
          </div>
        </div>
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" style="background:var(--cfp-primary);color:#fff;">
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทมิเตอร์น้ำ
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="watermetertype_save.php">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-7">
              ...
              <input type="text" class="form-control font-prompt" name="TypeName" id="fName"
                     placeholder="เช่น มิเตอร์น้ำดิจิทัล" maxlength="200" required>
            </div>
            <div class="col-12">
  ...
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import ประเภทมิเตอร์น้ำ จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="watermetertype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="watermetertype_template.php" class="cfp-import-template-link">
...
<form id="formDelete" method="POST" action="watermetertype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
<form id="formToggle" method="POST" action="watermetertype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
function openModal(id) {
    ...
    if (id === 0) {
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทมิเตอร์น้ำ';
        document.getElementById('fAction').value = 'create';
        document.getElementById('fID').value     = '0';
        document.getElementById('fCodeDisplay').value = 'ระบบสร้างให้อัตโนมัติ';
    } else {
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขประเภทมิเตอร์น้ำ';
