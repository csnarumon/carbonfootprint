<?php
/* ==============================================
   master/vehicletype.php
   จัดการประเภทยานพาหนะ — Admin (4), SustainAdmin (5)
   ============================================== */
...
/* ===== ดึงรายการประเภทยานพาหนะ ===== */
$res = sqlsrv_query($conn, "
    SELECT TypeID, TypeCode, TypeName, Description, SortOrder, IsActive
...
/* นับจำนวนประเภทที่ถูกใช้งานจริง (รวม CFP_Vehicle + CFP_Employee) — ต้องตรงกับ usageByType ด้านล่าง */
$resUsed = sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT VehicleTypeID) AS Cnt FROM (
        SELECT VehicleTypeID FROM CFP_Vehicle WHERE VehicleTypeID IS NOT NULL
        UNION
        SELECT VehicleTypeID FROM CFP_Employee WHERE VehicleTypeID IS NOT NULL
    ) AS used");
$rowUsed = sqlsrv_fetch_array($resUsed, SQLSRV_FETCH_ASSOC);
$usedCount = $rowUsed ? (int)$rowUsed['Cnt'] : 0;

/* จำนวนที่นำไปใช้จริงแยกตาม TypeID (รวม CFP_Vehicle + CFP_Employee) — ใช้ตัดสินใจว่าประเภทไหนลบได้ปลอดภัย */
$usageByType = array();
$resUsageDetail = sqlsrv_query($conn, "
    SELECT VehicleTypeID, COUNT(*) AS Cnt FROM (
        SELECT VehicleTypeID FROM CFP_Vehicle WHERE VehicleTypeID IS NOT NULL
        UNION ALL
        SELECT VehicleTypeID FROM CFP_Employee WHERE VehicleTypeID IS NOT NULL
    ) AS used GROUP BY VehicleTypeID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['VehicleTypeID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>ประเภทยานพาหนะ — ระบบบริหารจัดการคาร์บอนองค์กร</title>
    ...
    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
      <div>
        <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
          <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการประเภทยานพาหนะ
        </h5>
        <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
          การตั้งค่าข้อมูลพื้นฐาน › ประเภทยานพาหนะ
        </div>
      </div>
    ...
    <div class="row g-3 mb-3">
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-card-list" style="color:#1565C0;"></i>
            </div>
            <div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-truck" style="color:#E65100;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#E65100;line-height:1.1;">
                <?php echo $usedCount; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">ถูกใช้งานในยานพาหนะ</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-card-list me-2"></i>รายการประเภทยานพาหนะ
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มประเภทยานพาหนะ
            </button>
          </div>
        </div>
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" >
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทยานพาหนะ
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="vehicletype_save.php">
         ...
         style="background:#F0F0F0;color:var(--cfp-text-muted);">
            ...
            <div class="col-md-7">
              ...
              <input type="text" class="form-control font-prompt" name="TypeName" id="fName"
                     placeholder="เช่น รถยนต์ส่วนกลาง" maxlength="200" required>
            </div>
            <div class="col-12">
...
<div class="modal fade modal-cfp" id="modalImport" tabindex="-1" data-bs-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import ประเภทยานพาหนะ จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="vehicletype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="vehicletype_template.php" class="cfp-import-template-link">
...
<p style="font-size:0.72rem;color:var(--cfp-text-muted);">คอลัมน์: ชื่อ*, คำอธิบาย, ลำดับ </p>           <input type="file" id="importFile" name="import_file" accept=".xlsx" style="display:none;" onchange="handleFileSelect(this)">
          </div>
...
<form id="formDelete" method="POST" action="vehicletype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
<form id="formToggle" method="POST" action="vehicletype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
function openModal(id) {
    ...
    if (id === 0) {
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทยานพาหนะ';
        document.getElementById('fAction').value = 'create';
        document.getElementById('fID').value     = '0';
        document.getElementById('fCodeDisplay').value = 'ระบบสร้างให้อัตโนมัติ';
    } else {
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขประเภทยานพาหนะ';
