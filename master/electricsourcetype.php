<?php
/* ==============================================
   master/electrictype.php
   จัดการแหล่งไฟฟ้า — Admin (4), SustainAdmin (5)
   หมายเหตุ: ตาราง CFP_ElectricSourceType ใช้ Primary Key ชื่อ SourceID
   แต่คอลัมน์ข้อมูลใช้ TypeCode/TypeName (ผสมกัน ไม่เหมือน WaterSource
   ที่ใช้ SourceCode/SourceName ล้วน) มีฟิลด์เสริม GridFactor
   ============================================== */
...
$res = sqlsrv_query($conn, "
    SELECT SourceID, SourceCode, SourceName, GridFactor, Description, SortOrder, IsActive
...
/* นับจำนวนแหล่งไฟฟ้าที่ถูกใช้งานจริงใน CFP_ElectricMeter
   หมายเหตุ: ยังไม่ยืนยัน FK/ชื่อคอลัมน์จริงจาก schema — ใช้ชื่อตามรูปแบบเดียวกับ Water
   ถ้าตารางจริงไม่มีคอลัมน์ ElectricSourceID ให้แจ้งกลับเพื่อแก้ query นี้ */
$usedCount = 0;
$resUsed = @sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT ElectricSourceID) AS Cnt
    FROM CFP_ElectricMeter
    WHERE ElectricSourceID IS NOT NULL");
if ($resUsed !== false) {
    $rowUsed   = sqlsrv_fetch_array($resUsed, SQLSRV_FETCH_ASSOC);
    $usedCount = $rowUsed ? (int)$rowUsed['Cnt'] : 0;
}

/* จำนวนที่นำไปใช้จริงแยกตาม SourceID — ใช้ตัดสินใจว่าประเภทไหนลบได้ปลอดภัย */
$usageByType = array();
$resUsageDetail = @sqlsrv_query($conn, "
    SELECT ElectricSourceID, COUNT(*) AS Cnt FROM CFP_ElectricMeter
    WHERE ElectricSourceID IS NOT NULL GROUP BY ElectricSourceID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['ElectricSourceID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>แหล่งไฟฟ้า — ระบบบริหารจัดการคาร์บอนองค์กร</title>
     ...
     <!-- Page Header -->
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        <div>
          <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการแหล่งไฟฟ้า
          </h5>
          <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
            การตั้งค่าข้อมูลพื้นฐาน › แหล่งไฟฟ้า
          </div>
        </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-plug" style="color:#1565C0;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:var(--cfp-primary);line-height:1.1;">
                <?php echo $total; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">แหล่งไฟฟ้าทั้งหมด</div>
            </div>
          </div>
        </div>
        <div class="col-6 col-md-3">
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-lightning-charge" style="color:#E65100;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#E65100;line-height:1.1;">
                <?php echo $usedCount; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">ถูกใช้งานในมิเตอร์ไฟฟ้า</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-plug me-2"></i>รายการแหล่งไฟฟ้า
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          <div class="d-flex gap-2 flex-grow-1" style="max-width:560px;">
            ...
            <input type="text" id="fltKeyword" class="form-control font-prompt" style="font-size:0.85rem;padding-right:28px;"
                   placeholder="ค้นหารหัส / ชื่อแหล่งไฟฟ้า...">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มแหล่งไฟฟ้า
            </button>
          </div>
        </div>
          ...
          <table id="tblType" class="table table-bordered table-hover align-middle" style="width:100%">
            <thead>
              <tr>
                ...
                <th style="min-width:180px;">ชื่อแหล่งไฟฟ้า</th>
                <th class="cfp-col-hide" style="width:140px;">Grid Factor</th>
            ...
            <tbody>
              <?php foreach ($rows as $i => $r) {
                $usedN = $usageByType[(int)$r['SourceID']] ?? 0;
              ?>
              <tr data-status="<?php echo $r['IsActive'] ? '1' : '0'; ?>">
                ...
                <td class="cfp-col-hide" style="font-size:0.82rem;color:var(--cfp-text-muted);">
                  <?php echo ($r['GridFactor'] !== null) ? htmlspecialchars($r['GridFactor']) : '—'; ?>
                </td>
                  ...
                  <div class="cfp-action-group">
                    ...
                    <div class="cfp-act-secondary">
                      ...
                      <button class="btn btn-outline-warning btn-action cfp-act-del"
                              onclick="confirmDelete(<?php echo (int)$r['SourceID']; ?>, '<?php echo htmlspecialchars(addslashes($r['SourceName'])); ?>')"
                              title="ลบ">
...
<div class="modal fade" id="modalType" tabindex="-1" data-bs-backdrop="static">
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" style="background:var(--cfp-primary);color:#fff;">
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มแหล่งไฟฟ้า
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="electricsourcetype_save.php">
        ...
        <div class="modal-body">
          <div class="row g-3">
            <div class="col-md-5">
  <label class="form-label">รหัสแหล่งไฟฟ้า</label>
  <input type="text" class="form-control font-prompt" id="fCodeDisplay"
         ...
         style="background:#F0F0F0;color:var(--cfp-text-muted);">
            ...
            <div class="col-md-7">
              <label class="form-label form-required">ชื่อแหล่งไฟฟ้า</label>
              <input type="text" class="form-control font-prompt" name="SourceName" id="fName"
                     placeholder="เช่น ไฟฟ้าจากระบบสายส่ง (Grid)" maxlength="200" required>
            </div>
            <div class="col-md-6">
              <label class="form-label">Grid Emission Factor</label>
              <input type="number" class="form-control font-prompt" name="GridFactor" id="fGrid"
                     placeholder="เช่น 0.4999" step="0.0001" min="0">
            </div>
            <div class="col-md-6">
  ...
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import แหล่งไฟฟ้า จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="electrictype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="electricsourcetype_template.php" class="cfp-import-template-link">
          ...
          <div class="cfp-import-dropzone" id="dropZone" onclick="document.getElementById('importFile').click()">
            ...
            <p style="font-size:0.72rem;color:var(--cfp-text-muted);">คอลัมน์: ชื่อ*, Grid Factor, คำอธิบาย, ลำดับ (รหัสระบบสร้างให้อัตโนมัติ)</p>
          ...
          <div class="mt-2" style="font-size:0.78rem;color:var(--cfp-text-muted);">
            ...
            <ul class="mb-0 ps-3">
              ...
              <li>รหัสแหล่งไฟฟ้าระบบสร้างให้อัตโนมัติ ไม่ต้องกรอกในไฟล์
                <!-- Badge สีส้ม -->
...
<form id="formDelete" method="POST" action="electrictype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
  <input type="hidden" name="action" value="delete">
  <input type="hidden" name="SourceID" id="fdID" value="0">
</form>

<!-- Hidden Form สำหรับเปิด/ปิดใช้งานแบบ POST -->
<form id="formToggle" method="POST" action="electrictype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
function openModal(id) {
    ...
    document.getElementById('fGrid').value   = '';
    ...
    if (id === 0) {
        // เพิ่มใหม่
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มแหล่งไฟฟ้า';
        ...
        // แก้ไข
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขแหล่งไฟฟ้า';
        document.getElementById('fAction').value = 'update';
        document.getElementById('fID').value     = id;
        document.getElementById('fCodeDisplay').value = d.code;

        document.getElementById('fName').value    = d.name;
        document.getElementById('fGrid').value    = (d.grid !== null ? d.grid : '');
...
function confirmDelete(id, name) {
    Swal.fire({
        title: 'ยืนยันการลบ?',
        html: 'ต้องการลบ "<b>' + name + '</b>" ใช่หรือไม่',
        icon: 'warning',
...
function confirmToggle(id, currentActive, name) {
    ...
    Swal.fire({
        title: action + 'แหล่งไฟฟ้า?',
        html: 'แหล่งไฟฟ้า: <b>' + name + '</b>',
        icon: icon,
        showCancelButton: true,
