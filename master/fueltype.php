...
/* ==============================================
   master/fueltype.php
   จัดการประเภทเชื้อเพลิง — Admin (4), SustainAdmin (5)
   ============================================== */
...
/* ===== ดึงรายการประเภทเชื้อเพลิง ===== */
$res = sqlsrv_query($conn, "
    SELECT TypeID, TypeCode, TypeName, FuelGroup, DefaultUnit, Description, SortOrder, IsActive
    FROM CFP_FuelType
    ORDER BY SortOrder, TypeName");

if ($res === false) {
    echo '<pre style="background:#fff3f3;color:#900;padding:15px;font-size:13px;">';
...
/* นับจำนวนประเภทที่ถูกใช้งานจริง (อ้างอิงจาก CFP_Equipment + CFP_Vehicle) */
$resUsed = sqlsrv_query($conn, "
    SELECT COUNT(DISTINCT FuelTypeID) AS Cnt FROM (
        SELECT FuelTypeID FROM CFP_Equipment WHERE FuelTypeID IS NOT NULL
        UNION
        SELECT FuelTypeID FROM CFP_Vehicle WHERE FuelTypeID IS NOT NULL
    ) AS used");
$rowUsed   = sqlsrv_fetch_array($resUsed, SQLSRV_FETCH_ASSOC);
$usedCount = $rowUsed ? (int)$rowUsed['Cnt'] : 0;

/* จำนวนที่ใช้งานจริงแยกตาม TypeID — ใช้ตัดสินใจว่าประเภทไหนลบได้ปลอดภัย */
$usageByType = array();
$resUsageDetail = sqlsrv_query($conn, "
    SELECT FuelTypeID, COUNT(*) AS Cnt FROM (
        SELECT FuelTypeID FROM CFP_Equipment WHERE FuelTypeID IS NOT NULL
        UNION ALL
        SELECT FuelTypeID FROM CFP_Vehicle WHERE FuelTypeID IS NOT NULL
    ) AS used GROUP BY FuelTypeID");
if ($resUsageDetail) {
    while ($rU = sqlsrv_fetch_array($resUsageDetail, SQLSRV_FETCH_ASSOC)) {
        $usageByType[(int)$rU['FuelTypeID']] = (int)$rU['Cnt'];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>ประเภทเชื้อเพลิง — ระบบบริหารจัดการคาร์บอนองค์กร</title>
    ...
    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
      <div>
        <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
          <i class="bi bi-gear-wide-connected me-2" style="color:var(--cfp-green);"></i>จัดการประเภทเชื้อเพลิง
        </h5>
        <div style="font-size:0.78rem;color:var(--cfp-text-muted);margin-top:2px;">
          การตั้งค่าข้อมูลพื้นฐาน › ประเภทเชื้อเพลิง
        </div>
      </div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#E3F2FD;">
              <i class="bi bi-fuel-pump" style="color:#1565C0;"></i>
            </div>
            <div>
        ...
        <div class="col-6 col-md-3">
          <div class="cfp-card py-3 px-3 d-flex align-items-center gap-3 mb-0">
            <div class="kpi-icon-box" style="background:#FFF3E0;">
              <i class="bi bi-gear-wide-connected" style="color:#E65100;"></i>
            </div>
            <div>
              <div style="font-size:1.5rem;font-weight:700;color:#E65100;line-height:1.1;">
                <?php echo $usedCount; ?>
              </div>
              <div style="font-size:0.75rem;color:var(--cfp-text-muted);">ประเภทที่ถูกใช้งานอยู่</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cfp-card">
        <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
          <i class="bi bi-fuel-pump me-2"></i>รายการประเภทเชื้อเพลิง
        </div>
        ...
        <div class="cfp-page-toolbar mb-3" style="margin-bottom:14px;">
          <div class="d-flex gap-2 flex-grow-1" style="max-width:560px;">
            ...
            <input type="text" id="fltKeyword" class="form-control font-prompt" style="font-size:0.85rem;padding-right:28px;"
                   placeholder="ค้นหารหัส / ชื่อประเภท / กลุ่มเชื้อเพลิง...">
          ...
          <div class="cfp-page-toolbar-actions">
            ...
            <button class="btn-cfp-add" onclick="openModal(0)">
              <i class="bi bi-plus-circle"></i>เพิ่มประเภทเชื้อเพลิง
            </button>
          </div>
        </div>
          ...
          <table id="tblType" class="table table-bordered table-hover align-middle" style="width:100%">
            <thead>
              <tr>
                ...
                <th class="cfp-col-hide" style="width:140px;">กลุ่มเชื้อเพลิง</th>
                <th class="cfp-col-hide" style="width:100px;">หน่วยปกติ</th>
            ...
            <tbody>
              ...
              <tr data-status="<?php echo $r['IsActive'] ? '1' : '0'; ?>">
                ...
                <td class="cfp-col-hide" style="font-size:0.82rem;color:var(--cfp-text-muted);">
                  <?php echo htmlspecialchars($r['FuelGroup'] ?? '—'); ?>
                </td>
                <td class="cfp-col-hide" style="font-size:0.82rem;color:var(--cfp-text-muted);">
                  <?php echo htmlspecialchars($r['DefaultUnit'] ?? '—'); ?>
                </td>
                <td class="cfp-col-hide text-center"><?php echo (int)$r['SortOrder']; ?></td>
                <td class="cfp-col-hide text-center">
                  <?php if ($usedN > 0) { ?>
                    <span class="badge" style="background:#FFF3E0;color:#E65100;font-weight:600;" title="มีเครื่องจักร/ยานพาหนะนำไปใช้ประเภทนี้อยู่ — ลบไม่ได้ ต้องปิดใช้งานแทน">
    ...
    <div class="modal-content font-prompt">
      <div class="modal-header" >
        <h6 class="modal-title mb-0" id="modalTitle">
          <i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทเชื้อเพลิง
        </h6>
        <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form id="formType" method="POST" action="fueltype_save.php">
         ...
         style="background:#F0F0F0;color:var(--cfp-text-muted);">
            ...
            <div class="col-md-4">
              ...
              <input type="text" class="form-control font-prompt" name="TypeName" id="fName"
                     placeholder="เช่น Diesel" maxlength="200" required>
            </div>
            <div class="col-md-6">
              <label class="form-label">กลุ่มเชื้อเพลิง</label>
              <input type="text" class="form-control font-prompt" name="FuelGroup" id="fGroup"
                     placeholder="เช่น Fossil Fuel" maxlength="100">
            </div>
            <div class="col-md-6">
              <label class="form-label">หน่วยปกติ</label>
              <input type="text" class="form-control font-prompt" name="DefaultUnit" id="fUnit"
                     placeholder="เช่น ลิตร, กก." maxlength="50">
            </div>
            <div class="col-12">
...
<div class="modal fade modal-cfp" id="modalImport" tabindex="-1" data-bs-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content font-prompt">
      <div class="modal-header">
        <h6 class="modal-title"><i class="bi bi-file-earmark-spreadsheet"></i> Import ประเภทเชื้อเพลิง จาก Excel</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form id="formImport" method="POST" action="fueltype_import.php" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
        <div class="modal-body">

          <div class="mb-3 d-flex justify-content-between align-items-center">
            <label class="form-label mb-0">เลือกไฟล์ Excel <span style="color:var(--cfp-danger);">*</span></label>
            <a href="fueltype_template.php" class="cfp-import-template-link">
          ...
          <div class="cfp-import-dropzone" id="dropZone" onclick="document.getElementById('importFile').click()">
            ...
            <p style="font-size:0.72rem;color:var(--cfp-text-muted);">รหัส*, ชื่อ*, กลุ่มเชื้อเพลิง, หน่วยปกติ, คำอธิบาย, ลำดับ</p>
...
<form id="formDelete" method="POST" action="fueltype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
<form id="formToggle" method="POST" action="fueltype_save.php" style="display:none;">
  <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
...
function openModal(id) {
    ...
    if (id === 0) {
        // เพิ่มใหม่
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>เพิ่มประเภทเชื้อเพลิง';
        ...
        // แก้ไข
        var d = typeData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขประเภทเชื้อเพลิง';
        document.getElementById('fAction').value = 'update';
        document.getElementById('fID').value     = id;
        document.getElementById('fCodeDisplay').value = d.code;

        document.getElementById('fName').value   = d.name;
        document.getElementById('fGroup').value  = d.group;
