<?php
/* master/waste.php — รายการขยะ/ของเสีย (Scope 3 CAT5)
   แก้ไข: PDF Thumbnail, pending_deletes, ไม่มีหน้าแว๊บ, Filter 2 บรรทัด
*/
...
$resWT = @sqlsrv_query($conn, "SELECT TypeID, TypeCode, TypeName FROM CFP_WasteType WHERE IsActive=1 ORDER BY TypeName");
$wasteTypes = array();
if ($resWT) { while ($r = sqlsrv_fetch_array($resWT, SQLSRV_FETCH_ASSOC)) { $wasteTypes[] = $r; } }

$resWD = @sqlsrv_query($conn, "SELECT MethodID, MethodCode, MethodName FROM CFP_WasteDisposalMethod WHERE IsActive=1 ORDER BY MethodName");
$disposalMethods = array();
if ($resWD) { while ($r = sqlsrv_fetch_array($resWD, SQLSRV_FETCH_ASSOC)) { $disposalMethods[] = $r; } }

$resWS = @sqlsrv_query($conn, "SELECT SiteID AS DsID, SiteName AS DsName FROM CFP_WasteDisposalSite WHERE IsActive=1 ORDER BY SiteName");
$disposalSites = array();
if ($resWS) { while ($r = sqlsrv_fetch_array($resWS, SQLSRV_FETCH_ASSOC)) { $disposalSites[] = $r; } }

$res = @sqlsrv_query($conn, "
    SELECT w.WasteID, w.WasteCode, w.WasteName, w.SiteID, s.SiteName,
           w.WasteTypeID, wt.TypeName AS WasteTypeName,
           w.WasteDisposalMethodID, dm.MethodName AS DisposalMethodName,
           w.WasteDisposalSiteID, ds.SiteName AS DisposalSiteName,
           w.StorageLocation, w.Remark, w.IsActive
    FROM CFP_Waste w
    LEFT JOIN CFP_Site s ON s.SiteID = w.SiteID
    LEFT JOIN CFP_WasteType wt ON wt.TypeID = w.WasteTypeID
    LEFT JOIN CFP_WasteDisposalMethod dm ON dm.MethodID = w.WasteDisposalMethodID
    LEFT JOIN CFP_WasteDisposalSite ds ON ds.SiteID = w.WasteDisposalSiteID
    ORDER BY w.WasteCode");
$rows = array();
...
$wasteTypeFilterOptions = array();
$resWTypeAll = sqlsrv_query($conn, "SELECT TypeID, TypeName FROM CFP_WasteType WHERE IsActive=1 ORDER BY TypeName");
if ($resWTypeAll) {
    while ($row = sqlsrv_fetch_array($resWTypeAll, SQLSRV_FETCH_ASSOC)) {
        $wasteTypeFilterOptions[$row['TypeID']] = $row['TypeName'];
    }
}

$disposalMethodFilterOptions = array();
$resDMethodAll = sqlsrv_query($conn, "SELECT MethodID, MethodName FROM CFP_WasteDisposalMethod WHERE IsActive=1 ORDER BY MethodName");
if ($resDMethodAll) {
    while ($row = sqlsrv_fetch_array($resDMethodAll, SQLSRV_FETCH_ASSOC)) {
        $disposalMethodFilterOptions[$row['MethodID']] = $row['MethodName'];
    }
}
$map = array();
foreach ($rows as $r) {
    $map[$r['WasteID']] = array(
        ...
        'wtype'   => $r['WasteTypeID'] ? (int)$r['WasteTypeID'] : '',
        'dmethod' => $r['WasteDisposalMethodID'] ? (int)$r['WasteDisposalMethodID'] : '',
        'dsite'   => $r['WasteDisposalSiteID'] ? (int)$r['WasteDisposalSiteID'] : '',
        'loc'     => $r['StorageLocation'] ?? '',
        'remark'  => $r['Remark'] ?? '',
    );
}
$pageTitle = 'รายการขยะ/ของเสีย';
$pageIcon = 'trash3';
?>
<!DOCTYPE html>
<html lang="th">
<head>
    ...
    <title>รายการขยะ/ของเสีย — CFP</title>
...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
    <div>
        <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);">
            <i class="bi bi-trash3 me-2"></i>รายการขยะ / ของเสีย
        </h5>
        <div style="font-size:0.78rem;color:var(--cfp-text-muted)">
            ทะเบียนทรัพย์สิน (Scope 3) › รายการขยะ/ของเสีย
        </div>
    </div>
</div>
...
<div class="row g-3 mb-3">
    ...
    <div class="col-6 col-md-3">
        <div class="cfp-card py-2 px-3 text-center">
            <div style="font-size:1.6rem;font-weight:700;color:#7B1FA2"><?php echo count(array_unique(array_filter(array_column($rows, 'SiteID')))); ?></div>
            <div style="font-size:0.75rem;color:var(--cfp-text-muted)">Site ที่มีของเสีย</div>
        </div>
    </div>
</div>
...
<div class="cfp-card">
    ...
    <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
        <i class="bi bi-card-list me-2"></i>รายการขยะ/ของเสีย
    </div>
    ...
    <div class="cfp-page-toolbar">
        ...
        <div class="toolbar-row">
            <div class="toolbar-left">
                <input type="text" id="fltKeyword" class="form-control font-prompt"
                       placeholder="ค้นหารหัส / ชื่อขยะ / ตำแหน่งจัดเก็บ...">
            ...
            <div class="toolbar-actions">
                <button class="btn-cfp-add btn-sm" onclick="openModal(0)">
                    <i class="bi bi-plus-circle me-1"></i>เพิ่มรายการขยะ
                </button>
            </div>
        </div>

        <!-- บรรทัดที่ 2: Site + ประเภทขยะ + วิธีกำจัด -->
        <div class="toolbar-row toolbar-row-filters">
            <div class="toolbar-left">
                ...
                <select id="fltWType" class="form-select font-prompt">
                    <option value="">ประเภทขยะทั้งหมด</option>
                    <?php foreach ($wasteTypeFilterOptions as $id => $name) { ?>
                    <option value="<?php echo htmlspecialchars($name); ?>"><?php echo htmlspecialchars($name); ?></option>
                    <?php } ?>
                </select>
                <select id="fltDMethod" class="form-select font-prompt">
                    <option value="">วิธีกำจัดทั้งหมด</option>
                    <?php foreach ($disposalMethodFilterOptions as $id => $name) { ?>
                    <option value="<?php echo htmlspecialchars($name); ?>"><?php echo htmlspecialchars($name); ?></option>
                    <?php } ?>
                </select>
            </div>
...
<!-- ===== Modal ===== -->
<div class="modal fade" id="modalWaste" tabindex="-1" data-bs-backdrop="static">
        ...
        <div class="modal-content" style="font-family:'Prompt',sans-serif">
            <div class="modal-header" style="background:var(--cfp-primary);color:#fff">
                <h6 class="modal-title mb-0" id="modalTitle">
                    <i class="bi bi-trash3 me-2"></i>เพิ่มรายการขยะ/ของเสีย
                </h6>
                <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form id="formWaste" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            <label class="form-label form-required">รหัส</label>
                            <input type="text" class="form-control" name="WasteCode" id="fCode" 
                                   maxlength="50" required style="font-family:'Prompt',sans-serif;text-transform:uppercase">
                        </div>
                        <div class="col-md-9">
                            <label class="form-label form-required">ชื่อขยะ/ของเสีย</label>
                            <input type="text" class="form-control" name="WasteName" id="fName" 
                                   maxlength="300" required style="font-family:'Prompt',sans-serif">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Site / โรงงาน</label>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="WasteTypeID" id="fWType" style="font-family:'Prompt',sans-serif">
                                <option value="">— ไม่ระบุ —</option>
                                <?php foreach ($wasteTypes as $t) { ?>
                                <option value="<?php echo $t['TypeID']; ?>"><?php echo htmlspecialchars($t['TypeCode'] . ' — ' . $t['TypeName']); ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">วิธีกำจัด</label>
                            <select class="form-select" name="WasteDisposalMethodID" id="fDMethod" style="font-family:'Prompt',sans-serif">
                                <option value="">— ไม่ระบุ —</option>
                                <?php foreach ($disposalMethods as $m) { ?>
                                <option value="<?php echo $m['MethodID']; ?>"><?php echo htmlspecialchars($m['MethodCode'] . ' — ' . $m['MethodName']); ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">สถานที่กำจัด</label>
                            <select class="form-select" name="WasteDisposalSiteID" id="fDSite" style="font-family:'Prompt',sans-serif">
                                <option value="">— ไม่ระบุ —</option>
                                <?php foreach ($disposalSites as $ds) { ?>
                                <option value="<?php echo $ds['DsID']; ?>"><?php echo htmlspecialchars($ds['DsName']); ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-12">
                            <label class="form-label">ตำแหน่งจัดเก็บในโรงงาน</label>
                            <input type="text" class="form-control" name="StorageLocation" id="fLoc" 
                                   maxlength="300" style="font-family:'Prompt',sans-serif">
                        </div>
                        <div class="col-12">
                            <label class="form-label">หมายเหตุ</label>
                            <textarea class="form-control" name="Remark" id="fRemark" rows="2" 
                                      maxlength="500" style="font-family:'Prompt',sans-serif;resize:none"></textarea>
                        </div>
                        <div class="col-12">
...
var wasteData = <?php echo json_encode($map, JSON_UNESCAPED_UNICODE|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT); ?>;
...
function loadPreviewFromDB(assetID, callback) {
    ...
    $.ajax({
        ...
        method: 'POST',
        data: { asset_type: 'Waste', asset_id: assetID },
        dataType: 'json',
        async: false,
...
function _initFileInput(previewFiles, previewConfig) {
    ...
    $('#file-1').fileinput({
        ...
        uploadExtraData: function() {
            var data = {};
            $('#formWaste').serializeArray().forEach(function(item) {
    ...
    .on('filebatchuploadsuccess', function(event, data) {
        ...
        if (response && response.success) {
            ...
            var modalEl = document.getElementById('modalWaste');
...
function openModal(id) {
    var form = document.getElementById('formWaste');
    form.reset();
    pendingFiles = [];
    pendingDeletes = [];
    $('#cfp-file-error').hide().text('');
    ...
    if (id === 0) {
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-trash3 me-2"></i>เพิ่มรายการขยะ/ของเสีย';
        ...
        var d = wasteData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขรายการขยะ/ของเสีย';
        ...
        document.getElementById('fWType').value = d.wtype || '';
        document.getElementById('fDMethod').value = d.dmethod || '';
        document.getElementById('fDSite').value = d.dsite || '';
    ...
    $('#btnSave').prop('disabled', false).html('<i class="bi bi-check-circle me-1"></i>บันทึก');
    new bootstrap.Modal(document.getElementById('modalWaste')).show();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnSave').addEventListener('click', function() {
        ...
        if (!$('#fCode').val().trim() || !$('#fName').val().trim()) {
            Swal.fire({
                ...
                text: 'กรุณากรอกรหัสและชื่อของเสีย',
    ...
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        ...
        var type = $('#fltWType').val();
        var method = $('#fltDMethod').val();
        ...
        if (method !== '' && $(row).attr('data-method') !== method) return false;
        return true;
    });

    // ===== Event listeners: เมื่อเลือก filter ให้ redraw =====
    $('#fltStatus, #fltSite, #fltWType, #fltDMethod').on('change', function() {
        table.draw();
    });
