<?php
/* =============================================================
   data_entry/scope3.php
   บันทึกข้อมูล Scope 3 — Other Indirect GHG Emission
...
if (!$isSuperAdmin && $effRole === 1) {
    $resAcc = sqlsrv_query($conn,
        "SELECT CategoryIDs, SiteID FROM CFP_UserScopeAccess
         WHERE UserID=? AND ScopeNo=3 AND IsActive=1",
        array($userID));
    if ($resAcc) {
        $allowedCats  = array();
        $allowedSites = array();
...
$sqlItem = "
    ...
    WHERE i.ScopeNo = 3 AND i.IsActive = 1
";
...
/* ===== ดึงรายการทรัพย์สิน (Asset) สำหรับ dropdown ผูกกิจกรรม Scope 3 ===== */
/* Mapping: CAT4=ขนส่งต้นน้ำ->Vendor, CAT5=ของเสีย->Waste, CAT7=พนักงาน->Employee
   CAT อื่นๆ ไม่มี asset table (ตามเอกสาร Architecture ระบุ "ไม่ได้เก็บข้อมูล") */
$assetMap = array();

/* CAT4: Upstream Transportation -> CFP_Vendor (ตารางนี้ไม่มี SiteID จึงดึงทั้งหมดไม่กรอง Site) */
$resAssetVd = sqlsrv_query($conn,
    "SELECT VendorID AS AssetID, VendorCode AS AssetCode, VendorName AS AssetName
     FROM CFP_Vendor
     WHERE IsActive = 1
     ORDER BY VendorName");
if ($resAssetVd) {
    while ($r = sqlsrv_fetch_array($resAssetVd, SQLSRV_FETCH_ASSOC)) {
        $r['AssetType'] = 'Vendor';
        $assetMap[4][]  = $r;
    }
}

/* CAT5: Waste Generated -> CFP_Waste */
$resAssetWs = sqlsrv_query($conn,
    "SELECT WasteID AS AssetID, WasteCode AS AssetCode, WasteName AS AssetName
     FROM CFP_Waste
     WHERE SiteID = ? AND IsActive = 1
     ORDER BY WasteName",
    array($filterSite));
if ($resAssetWs) {
    while ($r = sqlsrv_fetch_array($resAssetWs, SQLSRV_FETCH_ASSOC)) {
        $r['AssetType'] = 'Waste';
        $assetMap[5][]  = $r;
    }
}
...
$resAssetEm = sqlsrv_query($conn,
    ...
    array($filterSite));
if ($resAssetEm) {
    while ($r = sqlsrv_fetch_array($resAssetEm, SQLSRV_FETCH_ASSOC)) {
        $r['AssetType'] = 'Employee';
        $assetMap[7][]  = $r;
    }
}
...
/* ===== Scope 3: จัดกลุ่มทุก category (รวมที่ยังไม่มี item) ===== */
$catLabels = array(
    1  => array('label' => '3.1 วัตถุดิบ',         'icon' => 'bi-box-seam',           'color' => '#D97706', 'full' => 'Purchased Goods & Services'),
    2  => array('label' => '3.2 ทุน',              'icon' => 'bi-building',           'color' => '#7C3AED', 'full' => 'Capital Goods'),
    3  => array('label' => '3.3 พลังงาน',          'icon' => 'bi-lightning-charge',   'color' => '#2AABB8', 'full' => 'Fuel & Energy Related'),
    4  => array('label' => '3.4 ขนส่ง(up)',        'icon' => 'bi-truck',              'color' => '#059669', 'full' => 'Upstream Transportation'),
    5  => array('label' => '3.5 ของเสีย',          'icon' => 'bi-trash3',             'color' => '#E05050', 'full' => 'Waste Generated'),
    6  => array('label' => '3.6 เดินทาง',          'icon' => 'bi-airplane',           'color' => '#0284C7', 'full' => 'Business Travel'),
    7  => array('label' => '3.7 พนักงาน',          'icon' => 'bi-person-walking',     'color' => '#7C3AED', 'full' => 'Employee Commuting'),
    8  => array('label' => '3.8 เช่า(up)',          'icon' => 'bi-building-up',        'color' => '#888',    'full' => 'Upstream Leased Assets'),
    9  => array('label' => '3.9 ขนส่ง(down)',      'icon' => 'bi-truck-front',        'color' => '#059669', 'full' => 'Downstream Transportation'),
    10 => array('label' => '3.10 แปรรูป',          'icon' => 'bi-gear-wide-connected','color' => '#D97706', 'full' => 'Processing of Sold Products'),
    11 => array('label' => '3.11 ใช้สินค้า',       'icon' => 'bi-bag-check',          'color' => '#2AABB8', 'full' => 'Use of Sold Products'),
    12 => array('label' => '3.12 สิ้นสุดอายุ',     'icon' => 'bi-recycle',            'color' => '#888',    'full' => 'End-of-Life Treatment'),
    13 => array('label' => '3.13 เช่า(down)',       'icon' => 'bi-building-down',      'color' => '#8B5CF6', 'full' => 'Downstream Leased Assets'),
    14 => array('label' => '3.14 แฟรนไชส์',       'icon' => 'bi-shop',               'color' => '#888',    'full' => 'Franchises'),
    15 => array('label' => '3.15 ลงทุน',           'icon' => 'bi-graph-up-arrow',     'color' => '#888',    'full' => 'Investments'),
);
/* สร้าง slot ทุก category ก่อน (empty array) แล้วค่อยใส่ items */
$grouped = array();
foreach (array_keys($catLabels) as $catKey) { $grouped[$catKey] = array(); }
foreach ($items as $item) {
    $cat = (int)$item['CategoryNo'];
    /* skip ถ้า CategoryNo ไม่อยู่ใน catLabels (ป้องกัน "อื่นๆ") */
    if (!isset($catLabels[$cat])) { continue; }
    $grouped[$cat][] = $item;
}
...
<title>บันทึกข้อมูล Scope 3 — ระบบบริหารจัดการคาร์บอนองค์กร</title>
...
.cat-tab-disabled { color:var(--cfp-text-muted) !important; cursor:default !important; opacity:.5; }
.cat-tab-disabled:hover { border-bottom-color:transparent !important; }
...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
  <div>
    <h5 class="mb-0 fw-bold" style="color:var(--cfp-primary);">
      <i class="bi bi-globe-asia-australia me-2" style="color:#8B5CF6;"></i>บันทึกข้อมูล Scope 3
    </h5>
    <div style="font-size:0.72rem;color:var(--cfp-text-muted);">
      Data Entry › Scope 3 — Other Indirect GHG Emission
...
<div class="row g-3 mb-3">
  ...
  <div class="col-6 col-md-3">
    <div class="kpi-card">
      ...
      <div class="kpi-lbl">รวม tCO₂e Scope 3</div>
    </div>
  </div>
</div>
...
<div class="cfp-card" style="padding:0;">
  ...
  $firstCat = true;
  foreach ($grouped as $catNo => $catItems) {
      $catInfo  = $catLabels[$catNo] ?? array('label'=>'อื่นๆ','icon'=>'bi-circle','color'=>'#888','full'=>'');
      $isEmpty  = empty($catItems);
      $dispStyle = (!$isEmpty && $firstCat) ? '' : 'display:none;';
  ?>
  <div class="cat-content" id="cat<?php echo $catNo; ?>" style="<?php echo $dispStyle; ?>">
    ...
    <div class="text-center py-4" style="color:var(--cfp-text-muted);">
      ...
      <div style="font-size:0.82rem;"><?php echo htmlspecialchars($catInfo['full']); ?></div>
      <div style="font-size:0.75rem;margin-top:4px;">ยังไม่มีรายการกิจกรรม — Admin ต้องเพิ่มใน <strong>master › รายการกิจกรรม</strong> ก่อน</div>
    </div>
    <?php } else { ?>
    <!-- ══ DESKTOP TABLE ══ -->
    ...
    <table class="entry-table">
      ...
      <tr class="<?php echo $rowLocked; ?>" data-item="<?php echo $iid; ?>" data-dataid="<?php echo $dataID; ?>"
        ...
        <td>
          ...
          <a class="attach-link attach-done" href="#" title="<?php echo htmlspecialchars($d['EvidenceFile']); ?>" onclick="openAttach(<?php echo $iid; ?>);return false;">
            <i class="bi bi-paperclip"></i><?php echo htmlspecialchars($d['EvidenceFileName'] ?? 'มีไฟล์'); ?>
            <span onclick="event.stopPropagation();deleteEvidence('<?php echo $iid; ?>',<?php echo (int)$d['DataID']; ?>);" style="color:#E05050;margin-left:4px;cursor:pointer;" title="ลบไฟล์"><i class="bi bi-x-circle"></i></span>
          </a>
...
<div class="sticky-bar mobile-only">
  <div>
    <div class="total-lbl">รวม Scope 3</div>
...
function confirmCancelDraft() {
    Swal.fire({
        ...
        html: 'ข้อมูล Scope 3 เดือนนี้จะถูกล้างทั้งหมด<br><small style="color:#6B7280;">ไม่สามารถกู้คืนได้</small>',
        showCancelButton: true,
