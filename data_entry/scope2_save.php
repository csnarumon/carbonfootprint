...
if(!$isSuperAdmin&&getEffectiveRole()===1){
    $resAcc=sqlsrv_query($conn,"SELECT CategoryIDs,SiteID FROM CFP_UserScopeAccess WHERE UserID=? AND ScopeNo=2 AND IsActive=1",array($userID));
    if($resAcc){
...
foreach($rows as $row){
    ...
    if(!$isSuperAdmin&&$allowedCats!==null){
        $resCat=sqlsrv_query($conn,
            "SELECT CategoryNo FROM CFP_ActivityItem WHERE ItemID=? AND ScopeNo=2 AND IsActive=1",
            array($itemID));
    ...
    if($res!==false){$successCount++;$savedRows[]=array('rowKey'=>(string)$itemID,'activityID'=>$activityID);}
