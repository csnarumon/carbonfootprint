...
if($action==='cancel_draft'){
    ...
    if(!$isSuperAdmin&&getEffectiveRole()===1){
        $resAccCd=sqlsrv_query($conn,"SELECT SiteID FROM CFP_UserScopeAccess WHERE UserID=? AND ScopeNo=1 AND IsActive=1",array($userID));
    ...
    $resHdr=sqlsrv_query($conn,
        "SELECT HeaderID,Status FROM CFP_MonthlyHeader WHERE SiteID=? AND YearMonth=? AND Scope='Scope1'",
    ...
    logAction($conn,'DATA_DELETE','CFP_MonthlyHeader',$headerID,$siteID,$yearMonth,'Scope1','ยกเลิก Draft ทั้งหมด');
...
if(!$isSuperAdmin&&getEffectiveRole()===1){
    $resAcc=sqlsrv_query($conn,"SELECT CategoryIDs,SiteID FROM CFP_UserScopeAccess WHERE UserID=? AND ScopeNo=1 AND IsActive=1",array($userID));
    if($resAcc){
...
if($hdrRow){
    ...
    /* Bug fix: CFP_MonthlyHeader ไม่มีคอลัมน์ UpdatedBy/UpdatedDate (มีแค่ CFP_ActivityData ที่มี)
       เดิม query นี้ error ทุกครั้ง (Invalid column name) ทำให้ Status ไม่เคยถูกอัปเดตเลย
       แต่ sqlsrv_query ไม่เช็ค return value จึงไม่มีใครเห็น error */
    ...
    $resIns=sqlsrv_query($conn,
        "INSERT INTO CFP_MonthlyHeader (SiteID,YearMonth,Scope,Status,ResponsibleName,ResponsibleDeptID,CreatedBy,CreatedDate) VALUES (?,?,'Scope1',?,?,?,?,GETDATE())",
...
/* ── Soft-delete แถวที่ผู้ใช้กดลบใน UI (ลบจริงเฉพาะแถวที่เป็นของ Header นี้เท่านั้น กัน user แก้ ID ส่งมาเอง) ── */
$deletedIds = $body['deletedIds'] ?? array();
if (!empty($deletedIds) && !empty($headerID)) {
    $deletedIds = array_map('intval', $deletedIds);
    $ph = implode(',', array_fill(0, count($deletedIds), '?'));
    $resDel = sqlsrv_query($conn,
        "UPDATE CFP_ActivityData SET IsActive=0, UpdatedBy=?, UpdatedDate=GETDATE() WHERE HeaderID=? AND ActivityID IN ($ph)",
        array_merge(array($userID, $headerID), $deletedIds));
    if ($resDel === false) {
        $e = sqlsrv_errors();
        jsonOut(false, 'ลบข้อมูลบางรายการไม่สำเร็จ: ' . ($e[0]['message'] ?? ''));
    }
}
...
foreach($rows as $row){
    ...
    if(!$isSuperAdmin&&$allowedCats!==null){
        $resCat=sqlsrv_query($conn,
            "SELECT CategoryNo FROM CFP_ActivityItem WHERE ItemID=? AND ScopeNo=1 AND IsActive=1",
            array($itemID));
    ...
    /* CAT3 Fugitive: ต้องระบุสารทำความเย็น (GWP ต่างกันตามชนิดสาร) */
    if ($catNo === 3 && !$assetID) {
        $errors[] = '"' . $actName . '" ต้องระบุสารทำความเย็น (CAT3)';
        $errorCount++; continue;
    }
