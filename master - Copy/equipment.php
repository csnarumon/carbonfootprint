...
/* ==============================================
   ...
   แก้ไข: Filter 2 บรรทัด รูปแบบเดียวกับ vehicle.php
   ============================================== */
...
$resAllEqCodes = sqlsrv_query($conn, "SELECT EquipmentCode, SiteID FROM CFP_Equipment");
$maxNumBySite  = array();
if ($resAllEqCodes) {
    while ($rx = sqlsrv_fetch_array($resAllEqCodes, SQLSRV_FETCH_ASSOC)) {
        $siteID = (int)$rx['SiteID'];
        $sc = $siteCodeMap[$siteID] ?? '';
        if (!$sc) continue;
        if (preg_match('/^EQ-' . preg_quote($sc, '/') . '-(\d+)$/i', $rx['EquipmentCode'], $m)) {
            $num = (int)$m[1];
            if (!isset($maxNumBySite[$siteID]) || $num > $maxNumBySite[$siteID]) {
                $maxNumBySite[$siteID] = $num;
            }
        }
    }
}
...
$resTypeAll = sqlsrv_query($conn, "SELECT TypeID, TypeName FROM CFP_EquipmentType WHERE IsActive=1 ORDER BY TypeName");
if ($resTypeAll) {
...
<head>
  ...
  <title>เครื่องจักร — GHG Management System</title>
  ...
  <div class="cfp-content">
    ...
    <div class="cfp-card">
      ...
      <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
        <i class="bi bi-card-list me-2"></i>รายการเครื่องจักร
      </div>
          ...
          <div class="toolbar-row">
              <div class="toolbar-left">
                  <input type="text" id="fltKeyword" class="form-control font-prompt"
                         placeholder="ค้นหารหัส / ชื่อเครื่องจักร / หมายเหตุ...">
      ...
      <form id="formEquipment" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="row g-3">
            ...
            <!-- แถว 2: ประเภท | เชื้อเพลิง | การเผาไหม้ -->
            <div class="col-md-4">
            ...
            <input type="hidden" name="Capacity"     id="fCap">
            <input type="hidden" name="CapacityUnit" id="fCapUnit">
            <input type="hidden" name="YearInstall"  id="fYear">
...
function updateAutoCode() {
    ...
    codeEl.value = 'EQ-' + sc + '-' + String(nextNum).padStart(3, '0');
}
...
function _initFileInput(previewFiles, previewConfig) {
    ...
    .on('filebatchuploadsuccess', function(event, data) {
        ...
        if (response && response.success) {
            ...
            var modalEl = document.getElementById('modalEquipment');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnSaveEquipment').addEventListener('click', function(e) {
        ...
        if (!$('#fName').val().trim()) {
            Swal.fire({ icon: 'warning', title: 'ข้อมูลไม่ครบ', text: 'กรุณากรอกชื่อเครื่องจักร', confirmButtonText: 'ตกลง', customClass: { popup: 'font-prompt' } });
            return;
        }
