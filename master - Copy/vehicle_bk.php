...
/* ==============================================
   ...
   รูปแบบเดียวกับ equipment.php (Krajee FileInput)
   ============================================== */
        ...
        <table id="tblVehicle" class="table table-bordered table-hover align-middle" style="width:100%">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <div class="fw-500"><?php echo htmlspecialchars($r['VehicleName']); ?></div>
                <?php if (!empty($r['LicensePlate'])) { ?>
                <div style="font-size:0.75rem;color:var(--cfp-text-muted);">
                  <i class="bi bi-card-text me-1"></i><?php echo htmlspecialchars($r['LicensePlate']); ?>
                  <?php if (!empty($r['YearModel'])) { echo '&nbsp;|&nbsp;ปี ' . $r['YearModel']; } ?>
                </div>
                <?php } ?>
              </td>
      ...
      <form id="formVehicle" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="row g-3">
            ...
            <!-- รหัส + ชื่อ -->
            <div class="col-md-4">
              <label class="form-label form-required">รหัสพาหนะ</label>
              <input type="text" class="form-control" name="VehicleCode" id="fCode"
                     placeholder="เช่น VH-001" maxlength="50" required
                     style="font-family:'Prompt',sans-serif;text-transform:uppercase">
            </div>
            <div class="col-md-8">
              <label class="form-label form-required">ชื่อพาหนะ / รุ่น</label>
              <input type="text" class="form-control" name="VehicleName" id="fName"
                     placeholder="เช่น Toyota Hilux Revo 2.4" maxlength="300" required
                     style="font-family:'Prompt',sans-serif">
            </div>

            <!-- ทะเบียน + ปีรุ่น + ขนาดเครื่องยนต์ -->
            <div class="col-md-4">
              <label class="form-label">ทะเบียนรถ</label>
              <input type="text" class="form-control" name="LicensePlate" id="fPlate"
                     placeholder="กข-1234" maxlength="50"
                     style="font-family:'Prompt',sans-serif;text-transform:uppercase">
            </div>
            <div class="col-md-3">
              <label class="form-label">ปีรุ่น (ค.ศ.)</label>
              <input type="number" class="form-control" name="YearModel" id="fYear"
            ...
            <div class="col-md-5">
              ...
              <input type="number" class="form-control" name="EngineSize" id="fEngine"
                     placeholder="2400" step="1" min="0"
                     style="font-family:'Prompt',sans-serif">
            </div>
            ...
            <div class="col-md-6">
              ...
              <select class="form-select" name="CombustionType" id="fCombustion"
                      style="font-family:'Prompt',sans-serif">
                <option value="Mobile">Mobile — เคลื่อนที่ (รถยนต์, forklift)</option>
        ...
        success: function(res) {
            if (res.success && res.images) {
                res.images.forEach(function(img) {
                    ...
                    previewConfig.push({ caption: img.caption, size: img.size, key: img.id, type: img.fileType || 'image' });
                });
            }
...
function _initFileInput(previewFiles, previewConfig) {
    ...
    .on('filebatchuploadsuccess', function(event, data) {
        ...
        if (response && response.success) {
            var bsModal = bootstrap.Modal.getInstance(document.getElementById('modalVehicle'));
    ...
    $('#file-1').off('change.cfpDupCheck').on('change.cfpDupCheck', function(e) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            var dupInDB      = savedPreviewConfig.some(function(c) { return (c.caption || '').toLowerCase() === name; });
            var dupInPending = pendingFiles.some(function(f) { return f.toLowerCase() === name; });
            var dupInBatch   = false;
            for (var j = 0; j < i; j++) { if (files[j].name.toLowerCase() === name) { dupInBatch = true; break; } }

            if (dupInDB || dupInPending || dupInBatch) { dupNames.push(files[i].name); }
        ...
        if (dupNames.length > 0) {
            ...
            showUploadErrorSwal(
                ...
                dupNames.map(function(n) { return '<i class="fa fa-file me-1"></i>' + n; }),
...
// ============================================================
// confirmDeleteFromDB — ลบไฟล์จาก DB
// ============================================================
function confirmDeleteFromDB(key) {
    Swal.fire({
        ...
        $.post('delete_asset_image.php', {
            fileid: key, asset_type: 'Vehicle', asset_id: currentAssetID
        }, function(res) {
            if (res.success) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnSaveVehicle').addEventListener('click', function(e) {
        ...
        if (!$('#fCode').val().trim() || !$('#fName').val().trim()) {
            Swal.fire({ icon: 'warning', title: 'ข้อมูลไม่ครบ', text: 'กรุณากรอกรหัสและชื่อยานพาหนะ', confirmButtonText: 'ตกลง', customClass: { popup: 'font-prompt' } });
            return;
        }
