<?php
/* ==============================================
   master/vehicletype_import.php
   รับไฟล์ Excel (.xlsx) แล้วนำเข้าข้อมูลประเภทยานพาหนะ
...
/* ===== Helper: generate รหัสอัตโนมัติ รูปแบบ VT-0001 (เหมือนใน vehicletype_save.php) ===== */
...
$res = sqlsrv_query($conn, "SELECT TypeCode, TypeName FROM CFP_VehicleType");
$existingCodes = array();
$existingNames = array();
...
logImportResult($conn, 'CFP_VehicleType', $fileName, $totalRows, $successCount, $failCount, $skipCount, $errors);

if ($successCount > 0) {
    logAction($conn, 'DATA_IMPORT', 'CFP_VehicleType', null, null, null, null,
