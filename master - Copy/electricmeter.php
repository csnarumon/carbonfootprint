<?php
/* master/electricmeter.php — ทะเบียนมิเตอร์ไฟฟ้า (Krajee FileInput)
   แก้ไข: PDF Thumbnail, pending_deletes, ไม่มีหน้าแว๊บ, Filter 2 บรรทัด (แบบ watermeter.php)
*/
...
$resAllEMCodes = sqlsrv_query($conn, "SELECT MeterCode, SiteID FROM CFP_ElectricMeter");
$maxNumBySite  = array();
if ($resAllEMCodes) {
    while ($rx = sqlsrv_fetch_array($resAllEMCodes, SQLSRV_FETCH_ASSOC)) {
        $sid = (int)$rx['SiteID'];
        $sc  = $siteCodeMap[$sid] ?? '';
        if (!$sc) continue;
        if (preg_match('/^EM-' . preg_quote($sc, '/') . '-(\d+)$/i', $rx['MeterCode'], $m)) {
            $num = (int)$m[1];
...
$resEMT = @sqlsrv_query($conn,"SELECT TypeID,TypeCode,TypeName FROM CFP_ElectricMeterType WHERE IsActive=1 ORDER BY TypeName");
$emTypes = array();
if ($resEMT) { while ($r = sqlsrv_fetch_array($resEMT,SQLSRV_FETCH_ASSOC)) { $emTypes[] = $r; } }

$resES = @sqlsrv_query($conn,"SELECT SourceID,SourceCode,SourceName FROM CFP_ElectricSourceType WHERE IsActive=1 ORDER BY SourceName");
$eSources = array();
if ($resES) { while ($r = sqlsrv_fetch_array($resES,SQLSRV_FETCH_ASSOC)) { $eSources[] = $r; } }

$res = sqlsrv_query($conn,"
    SELECT m.MeterID,m.MeterCode,m.MeterName,m.SiteID,s.SiteName,
           m.ElectricMeterTypeID,mt.TypeName AS MeterTypeName,
           m.ElectricSourceID,es.SourceName AS ESourceName,
           m.MeterNo,m.Voltage,m.Phase,m.MaxLoad,
           m.InstallDate,m.Location,m.Remark,m.IsActive
    FROM CFP_ElectricMeter m
    LEFT JOIN CFP_Site s ON s.SiteID=m.SiteID
    LEFT JOIN CFP_ElectricMeterType mt ON mt.TypeID=m.ElectricMeterTypeID
    LEFT JOIN CFP_ElectricSourceType es ON es.SourceID=m.ElectricSourceID
...
$resMTypeAll = sqlsrv_query($conn, "SELECT TypeID, TypeName FROM CFP_ElectricMeterType WHERE IsActive=1 ORDER BY TypeName");
if ($resMTypeAll) {
...
$electricSourceFilterOptions = array();
$resESourceAll = sqlsrv_query($conn, "SELECT SourceID, SourceName FROM CFP_ElectricSourceType WHERE IsActive=1 ORDER BY SourceName");
if ($resESourceAll) {
    while ($row = sqlsrv_fetch_array($resESourceAll, SQLSRV_FETCH_ASSOC)) {
        $electricSourceFilterOptions[$row['SourceID']] = $row['SourceName'];
    }
}
...
foreach ($rows as $r) {
    $map[$r['MeterID']] = array(
        ...
        'emtype'  => $r['ElectricMeterTypeID']?(int)$r['ElectricMeterTypeID']:'',
        'esrc'    => $r['ElectricSourceID']?(int)$r['ElectricSourceID']:'',
        'mno'     => $r['MeterNo']??'',
        'volt'    => $r['Voltage']?(float)$r['Voltage']:'',
        'phase'   => $r['Phase']?(int)$r['Phase']:'',
        'maxload' => $r['MaxLoad']?(float)$r['MaxLoad']:'',
        'install' => $r['InstallDate']?date_format($r['InstallDate'],'Y-m-d'):'',
        'loc'     => $r['Location']??'', 'remark' => $r['Remark']??'',
    );
}

$pageTitle = 'มิเตอร์ไฟฟ้า';
$pageIcon  = 'lightning-charge';
?>
<!DOCTYPE html><html lang="th"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>มิเตอร์ไฟฟ้า — CFP</title>
...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
  <div>
    <h5 class="mb-0 fw-600" style="color:var(--cfp-primary);"><i class="bi bi-<?php echo $pageIcon; ?> me-2"></i>จัดการ<?php echo $pageTitle; ?></h5>
    <div style="font-size:0.78rem;color:var(--cfp-text-muted)">ทะเบียนทรัพย์สิน (Scope 2) › <?php echo $pageTitle; ?></div>
  </div>
</div>
...
<div class="cfp-card">
    ...
    <div style="font-size:0.92rem;font-weight:600;color:var(--cfp-primary);margin-bottom:12px;">
        <i class="bi bi-card-list me-2"></i>รายการมิเตอร์ไฟฟ้า
    </div>
    ...
    <div class="cfp-page-toolbar">
        ...
        <div class="toolbar-row">
            ...
            <div class="toolbar-actions">
                <button class="btn-cfp-add btn-sm" onclick="openModal(0)">
                    <i class="bi bi-plus-circle me-1"></i>เพิ่มมิเตอร์ไฟฟ้า
                </button>
            </div>
        </div>

        <!-- บรรทัดที่ 2: Site + ประเภทมิเตอร์ + แหล่งไฟฟ้า -->
        <div class="toolbar-row toolbar-row-filters">
            <div class="toolbar-left">
                ...
                <select id="fltEMType" class="form-select font-prompt">
                    <option value="">ประเภทมิเตอร์ทั้งหมด</option>
                    <?php foreach ($meterTypeFilterOptions as $id => $name) { ?>
                    <option value="<?php echo htmlspecialchars($name); ?>"><?php echo htmlspecialchars($name); ?></option>
                    <?php } ?>
                </select>
                <select id="fltESrc" class="form-select font-prompt">
                    <option value="">แหล่งไฟฟ้าทั้งหมด</option>
                    <?php foreach ($electricSourceFilterOptions as $id => $name) { ?>
                    <option value="<?php echo htmlspecialchars($name); ?>"><?php echo htmlspecialchars($name); ?></option>
                    <?php } ?>
                </select>
            </div>
    ...
    <div class="table-responsive">
        <table id="tblEM" class="table table-bordered table-hover align-middle" style="width:100%;font-size:0.85rem">
            <thead>
                <tr>
                    ...
                    <th class="cfp-col-hide" style="width:120px">แหล่งไฟฟ้า</th>
                    <th class="cfp-col-hide" style="width:90px">แรงดัน</th>
            ...
            <tbody>
                ...
                <tr data-status="<?php echo $r['IsActive'] ? 'ใช้งาน' : 'ปิด'; ?>"
                    ...
                    data-source="<?php echo htmlspecialchars($r['ESourceName'] ?? ''); ?>">
                    ...
                    <td class="cfp-col-hide"><?php echo htmlspecialchars($r['ESourceName']??'—'); ?></td>
                    <td class="cfp-col-hide" style="font-size:0.8rem"><?php if($r['Voltage']){echo $r['Voltage'].'V';} if($r['Phase']){echo ' '.$r['Phase'].'φ';} if(!$r['Voltage']&&!$r['Phase'])echo '—'; ?></td>
...
<!-- Modal -->
<div class="modal fade" id="modalEM" tabindex="-1" data-bs-backdrop="static">
...
<div class="modal-header" style="background:var(--cfp-primary);color:#fff">
  <h6 class="modal-title mb-0" id="modalTitle"><i class="bi bi-lightning-charge me-2"></i>เพิ่มมิเตอร์ไฟฟ้า</h6>
  <button class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
</div>
<form id="formEM" method="POST" enctype="multipart/form-data">
...
<div class="modal-body"><div class="row g-3">
  ...
  <div class="col-md-6"><label class="form-label">ประเภทมิเตอร์</label>
    <select class="form-select" name="ElectricMeterTypeID" id="fEMType" style="font-family:'Prompt',sans-serif">
      <option value="">— ไม่ระบุ —</option>
      <?php foreach ($emTypes as $t) { ?><option value="<?php echo $t['TypeID']; ?>"><?php echo htmlspecialchars($t['TypeCode'].' — '.$t['TypeName']); ?></option><?php } ?>
    </select></div>
  <div class="col-md-6"><label class="form-label">แหล่งไฟฟ้า</label>
    <select class="form-select" name="ElectricSourceID" id="fESrc" style="font-family:'Prompt',sans-serif">
      <option value="">— ไม่ระบุ —</option>
      <?php foreach ($eSources as $es) { ?><option value="<?php echo $es['SourceID']; ?>"><?php echo htmlspecialchars($es['SourceCode'].' — '.$es['SourceName']); ?></option><?php } ?>
    </select></div>
  ...
  <div class="col-md-3"><label class="form-label">แรงดัน (V)</label>
    <input type="number" class="form-control" name="Voltage" id="fVolt" step="0.01" min="0" placeholder="220" style="font-family:'Prompt',sans-serif"></div>
  <div class="col-md-2"><label class="form-label">Phase</label>
    <select class="form-select" name="Phase" id="fPhase" style="font-family:'Prompt',sans-serif">
      <option value="">—</option><option value="1">1 Phase</option><option value="3">3 Phase</option>
    </select></div>
  <div class="col-md-3"><label class="form-label">Max Load (kW)</label>
    <input type="number" class="form-control" name="MaxLoad" id="fMaxLoad" step="0.01" min="0" style="font-family:'Prompt',sans-serif"></div>
  <div class="col-md-4"><label class="form-label">วันติดตั้ง</label>
    <input type="date" class="form-control" name="InstallDate" id="fInstall" style="font-family:'Prompt',sans-serif"></div>
  <div class="col-md-8"><label class="form-label">ตำแหน่งติดตั้ง</label>
...
var emData    = <?php echo json_encode($map, JSON_UNESCAPED_UNICODE|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT); ?>;
...
function updateAutoCode() {
    ...
    codeEl.value = 'EM-' + sc + '-' + String(nextNum).padStart(3, '0');
}
...
function loadPreviewFromDB(assetID, callback) {
    ...
    $.ajax({
        ...
        method: 'POST',
        data: { asset_type: 'ElectricMeter', asset_id: assetID },
        dataType: 'json',
        async: false,
...
function _initFileInput(previewFiles, previewConfig) {
    ...
    $('#file-1').fileinput({
        theme: 'fas',
        uploadUrl: 'electricmeter_save.php',
        uploadAsync: false,
        showUpload: false,
        showRemove: false,
        autoUpload: false,
        ...
        uploadExtraData: function() {
            var data = {};
            $('#formEM').serializeArray().forEach(function(item) {
    ...
    .on('filebatchuploadsuccess', function(event, data) {
        ...
        if (response && response.success) {
            ...
            var modalEl = document.getElementById('modalEM');
...
function openModal(id) {
    var form = document.getElementById('formEM');
    form.reset();
    pendingFiles = [];
    pendingDeletes = [];
    $('#cfp-file-error').hide().text('');
    ...
    if (id === 0) {
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-lightning-charge me-2"></i>เพิ่มมิเตอร์ไฟฟ้า';
        ...
        var d = emData[id];
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil-square me-2"></i>แก้ไขมิเตอร์ไฟฟ้า';
        ...
        document.getElementById('fEMType').value = d.emtype || '';
        document.getElementById('fESrc').value = d.esrc || '';
        document.getElementById('fMNo').value = d.mno || '';
        document.getElementById('fVolt').value = d.volt || '';
        document.getElementById('fPhase').value = d.phase || '';
        document.getElementById('fMaxLoad').value = d.maxload || '';
    ...
    $('#btnSave').prop('disabled', false).html('<i class="bi bi-check-circle me-1"></i>บันทึก');
    new bootstrap.Modal(document.getElementById('modalEM')).show();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnSave').addEventListener('click', function() {
        ...
        if (!$('#fName').val().trim()) {
            Swal.fire({
                ...
                text: 'กรุณากรอกชื่อมิเตอร์ไฟฟ้า',
    ...
    var table = $('#tblEM').DataTable({
        ...
        drawCallback: function () { cfpInitMobileExpand('tblEM'); }
    });
    ...
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        ...
        var type = $('#fltEMType').val();
        var source = $('#fltESrc').val();
    ...
    $('#fltStatus, #fltSite, #fltEMType, #fltESrc').on('change', function() {
        table.draw();
    });
...
function confirmToggle(id, isActive, name) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            f.method = 'POST';
            f.action = 'electricmeter_save.php';
