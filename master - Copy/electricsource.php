<?php
/* ==============================================
   master/lookup_electricsource.php
   แหล่งไฟฟ้า — CFP_ElectricSource
   ============================================== */
...
$pageIcon  = 'plug';
$tableName = 'CFP_ElectricSource';
$importType = 'ElectricSource';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save') {
    ...
    $gridfactor = ($_POST['GridFactor'] !== '') ? (float)$_POST['GridFactor'] : null;
    ...
    if ($typeID > 0) {
        ...
        $sql = "UPDATE $tableName SET
                    TypeCode = ?, TypeName = ?, Description = ?, GridFactor = ?,
                    SortOrder = ?, IsActive = ?,
                    UpdatedBy = ?, UpdatedDate = GETDATE()
                WHERE SourceID = ?";
        $params = array($typeCode, $typeName, $description, $gridfactor, $sortOrder, $isActive, $userID, $typeID);
        $oldAction = 'UPDATE_LOOKUP';
    } else {
        /* INSERT */
        $sql = "INSERT INTO $tableName (TypeCode, TypeName, Description, GridFactor, SortOrder, IsActive, CreatedBy, CreatedDate)
                VALUES (?, ?, ?, ?, ?, ?, ?, GETDATE())";
        $params = array($typeCode, $typeName, $description, $gridfactor, $sortOrder, $isActive, $userID);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'import') {
    ...
    /* TODO: ใช้ PhpSpreadsheet อ่านไฟล์ที่ $_FILES['importFile']
       วน insert ทีละแถวลง CFP_ElectricSource
       บันทึกผลลง CFP_ImportLog (ImportType = 'ElectricSource')
...
$sql  = "SELECT SourceID, TypeCode, TypeName, Description, GridFactor, SortOrder, IsActive
...
<head>
  ...
  <title><?php echo $pageTitle; ?> — GHG Management System</title>
    ...
    <div class="modal-content">
      ...
      <form id="formLookup">
        <div class="modal-body">
          ...
          <div class="mb-3">
            <label class="form-label">ค่า Grid Emission Factor</label>
            <input type="number" step="0.01" class="form-control" id="GridFactor" name="GridFactor">
          </div>
...
function editRow(row) {
    ...
    document.getElementById('GridFactor').value = row.GridFactor || '';
...
document.getElementById('formLookup').addEventListener('submit', function (e) {
    ...
    fetch('lookup_electricsource.php', { method: 'POST', body: formData })
...
function downloadTemplate() {
    ...
    window.location.href = '/carbonfootprint/templates/template_electricsource.xlsx';
}
